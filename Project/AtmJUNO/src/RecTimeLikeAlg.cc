...
:   AlgBase(name)
{
	...
	//lizy::2018-06-08 Migrate to Minimizer
    m_Minimize = "Minuit2_Migrad";
	...
	//lizy::2018-06-08 Migrate to Minimizer
	LogDebug<<"Minimize:"<<m_Minimize<<std::endl;
   Time_1hit = 0;
   Time_2hit = 0;
   Time_3hit = 0;
   Time_4hit = 0;
...
bool RecTimeLikeAlg::initialize()
{
	//lizy::2018-06-08 Migrate to Minimizer
	fcn = new MyFCN(this);
	if (m_Minimize == "TFitterMinuit"){
		// recminuit = new TFitterMinuit();
		// recminuit->SetMinuitFCN(fcn);
		// recminuit->SetPrintLevel(0);
            LogError << "TFitterMinuit is not supported in ROOT6." << std::endl;
            return false;
	}
	else if (m_Minimize == "Minuit2_Migrad"){
		recminimizer = ROOT::Math::Factory::CreateMinimizer("Minuit2", "Migrad");
	}
	else if (m_Minimize == "Minuit2_Combined"){
		recminimizer = ROOT::Math::Factory::CreateMinimizer("Minuit2", "Combined");
	}
	else if (m_Minimize == "GSL_BFGS"){
		recminimizer = ROOT::Math::Factory::CreateMinimizer("GSLMultiMin", "BFGS");
	}
	else if (m_Minimize == "GSL_BFGS2"){
		recminimizer = ROOT::Math::Factory::CreateMinimizer("GSLMultiMin", "BFGS2");
	}
	else if (m_Minimize == "GSL_MultiFit"){
		recminimizer = ROOT::Math::Factory::CreateMinimizer("GSLMultiFit", "");
	}
	else if (m_Minimize == "GSL_SimAn"){
		recminimizer = ROOT::Math::Factory::CreateMinimizer("GSLSimAn", "");
	}
	else {
		LogError << "Minimize Method not correct" << std::endl;
		return false;
	}
...
//    if(PMT_ID.at(i) == 5)std::cout<<"time-tof:"<<Readout_hit_time.at(i) - timeflight<<",bin cut"<<BIN_CUT[NBin]<<std::endl;
   ...
   if (m_Minimize == "TFitterMinuit"){
	   ...
	   recminimizer->SetVariable(0, "n", 5000, 0.5);
	   recminimizer->SetFixedVariable(1, "xpos", x_fit);
	   recminimizer->SetFixedVariable(2, "ypos", y_fit);
	   recminimizer->SetFixedVariable(3, "zpos", z_fit);

	   recminimizer->SetUpperLimitedVariable(0, "n", 5000, 0.5, 100000);
	   recminimizer->SetLowerLimitedVariable(0, "n", 5000, 0.5, 0);


	   recminimizer->Minimize();
...
//   std::cout<<tmp_ve_x<<std::endl; 
   ...
   for(int iteration = 0; iteration < 100; iteration++){//100
       ...
       //lizy :: fix center region influence by min_hit_time (min_hit_time is large in the center, need to extend time search range).
       for(int bin_t = -4;bin_t<10;bin_t++){//-4,10

...
void Data::Initialize(std::string config)
{
  ...
  alpha_str = GetValue("ALPHA",config.c_str());
  gamma_str = GetValue("GAMMA",config.c_str());
  TString str_alpha = alpha_str;
  TString str_gamma = gamma_str;
  alphamod = atof(str_alpha);
  gammamod = atof(str_gamma);
}
...
void Data::GetMAT(Int_t binsE, Float_t lowedgeE, Float_t upedgeE, Int_t binsNPE, Float_t lowedgeNPE, Float_t upedgeNPE, std::string outfold) {
  ...
  //for (Long64_t jentry=0; jentry<20000;jentry++) {
    ...
    */
    ...
    if ((CCint==1 && abs(init_lepID)==(id_family)) || (NCint==1 && abs(init_lepID)==(id_family+1))) Ev_sgn->Fill(log10(nuEnergy),osc_weight);
...
void Data::GetDATA(Int_t binsE, Float_t lowedgeE, Float_t upedgeE, Int_t binsNPE, Float_t lowedgeNPE, Float_t upedgeNPE, std::string outfold)
{
  ...
  */
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    */
    osc_weight = osc_weight*((1.0 + alphamod)*(pow((nuEnergy/1000.),gammamod)));

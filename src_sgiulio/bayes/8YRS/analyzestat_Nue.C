void analyzestat_Nue(){
  ...
  std::ofstream rel_err("outputs_stat_Nue/staterror_Nue_7_7.txt");
  TFile *f_out = new TFile("outputs_stat_Nue/mod_NPE_7_7.root", "RECREATE");
  ...
  for (int k=1; k<=1000; k++) {
    sprintf(input_file,"/storage/DATA-02/juno/Users/sgiulio/Atmo/analysis/unfolding/bayes/outputs_stat_Nue/Unf_spectrum-%i_7_7.txt",k);
  ...
  std::cout << "file outputs_stat_Nue/mod_NPE_7_7.root created" << std::endl;
  f_out->Close();
}

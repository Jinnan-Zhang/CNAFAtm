void ALL_systosc_Numu(){
  ...
  std::ofstream rel_err("/storage/DATA-02/juno/Users/sgiulio/Atmo/analysis/unfolding/bayes/outputs_systosc_Numu/systosc_err_Numu_7_8.txt");
  TFile *f_out = new TFile("/storage/DATA-02/juno/Users/sgiulio/Atmo/analysis/unfolding/bayes/outputs_systosc_Numu/systosc_Numu_7_8.root", "RECREATE");
  ...
  Float_t scale_factor[7] = {0.07,0.03,0.01,3.5E-3,0.7E-3,2.0E-4,0.52E-4};
  ...
  flux_true->GetYaxis()->SetRangeUser(1.E-6,0.7);
  int index;
  for (int k=0; k<=1000; k++) {
    sprintf(input_file,"/storage/DATA-02/juno/Users/sgiulio/Atmo/analysis/unfolding/bayes/outputs_systosc_Numu/Unf_spectrum-%i_7_8.txt",k);
  ...
  for(int ii=0; ii<NBINS; ii++) {
    ...
    std::cout<<fit_sigma[ii]/fit_peak[ii]<<"\t"<<abs(fit_peak[ii]-unf_arr[ii])/unf_arr[ii]<<std::endl;
    histos_c[ii]->cd();
    Flux_bin_r_copy_h[ii]->Draw();
    true_flux_l[ii]->Draw();
    fit_flux_l[ii]->Draw();
  ...
  std::cout << "file outputs_systosc_Numu/systosc_Numu_7_8.root created" << std::endl;
  f_out->Close();
}

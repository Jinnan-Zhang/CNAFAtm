void All_systxsec_Numu(){
  ...
  std::ofstream rel_err("/storage/DATA-02/juno/Users/sgiulio/Atmo/analysis/unfolding/bayes/outputs_xsec_Numu/xsecerror_Numu_7_8.txt");
  TFile *f_out = new TFile("outputs_xsec_Numu/xsec_NPE_7_8.root", "RECREATE");
  ...
  mod_ratio.ResizeTo(vec_unf_arr.size());
  ...
  for (int k=1; k<=1000; k++) {
    sprintf(input_file,"/storage/DATA-02/juno/Users/sgiulio/Atmo/analysis/unfolding/bayes/outputs_xsec_Numu/Unf_spectrum-%i_7_8.txt",k);
  ...
  for(int ii=0; ii<NBINS; ii++) {
    ...
    fit_flux_l[ii]->Draw();
    histos_c[ii]->Write();
    mod_ratio[ii] = ( fit_peak[ii]/unf_arr[ii] );
  ...
  TGraphErrors *fitratio = new TGraphErrors(unf_bins,mod_ratio,err0,err_ratio);
  ...
  std::cout << "file outputs_xsec_Numu/xsec_NPE_7_8.root created" << std::endl;
  f_out->Close();
}

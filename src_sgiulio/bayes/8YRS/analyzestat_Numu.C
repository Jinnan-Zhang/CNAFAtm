void analyzestat_Numu(){
  std::ifstream true_array("/storage/DATA-02/juno/Users/sgiulio/Atmo/analysis/unfolding/bayes/output_Numu_osc/Unf_spectrum_7_8.txt");
  ...
  std::ofstream rel_err("/storage/DATA-02/juno/Users/sgiulio/Atmo/analysis/unfolding/bayes/outputs_stat_Numu/staterror_Numu_7_8.txt");
  TFile *f_out = new TFile("outputs_stat_Numu/mod_NPE_7_8.root", "RECREATE");
  ...
  stat_ratio.ResizeTo(vec_unf_arr.size());
  ...
  Float_t scale_factor[7] = {0.07,0.04,0.01,3.5E-3,1.E-3,2.7E-4,0.8E-4};
  ...
  TH2F *Flux_var_h = new TH2F("Flux_var_h","",7,-0.3,1.2,200,1.E-7,0.1);
  ...
  for (int k=1; k<=1000; k++) {
    sprintf(input_file,"/storage/DATA-02/juno/Users/sgiulio/Atmo/analysis/unfolding/bayes/outputs_stat_Numu/Unf_spectrum-%i_7_8.txt",k);
  ...
  for(int ii=0; ii<NBINS; ii++) {
    ...
    fit_flux_l[ii]->Draw();
    histos_c[ii]->Write();
    stat_ratio[ii] = ( fit_peak[ii]/unf_arr[ii] );
  ...
  TGraphErrors *fitratio = new TGraphErrors(unf_bins,stat_ratio,err0,err_ratio);
  ...
  fitratio->GetXaxis()->SetTitle("log_{10}(E_{#nu} [GeV])");
  fitratio->GetYaxis()->SetRangeUser(0.83,1.18);
  fitratio->SetTitle("");
  ...
  std::cout << "file outputs_stat_Numu/mod_NPE_7_8.root created" << std::endl;
  f_out->Close();
}

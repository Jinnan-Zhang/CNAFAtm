void analyzestat_Numu(){
  ...
  std::ifstream true_array("output_Numu_osc/Unf_spectrum_7_8.txt");
  //std::ifstream true_array("../output_Numu_osc_NEW/Unf_spectrum_7_8.txt");
  std::ifstream bins("../../matrix/likelihood/5YRS/output_Numu_osc/energybins_Numu_7.txt");
  std::ofstream rel_err("outputs_stat_Numu/staterror_Numu_7_8.txt");
  ...
  for (int qq=0;qq<NBINS;qq++) {                                                                                                   
    ...
    true_flux_l[qq]->SetLineColor(kBlue);
  }
  ...
  flux_true->SetMarkerColor(kBlue);
  ...
  for (int k=1; k<=1000; k++) {
    sprintf(input_file,"outputs_stat_Numu/Unf_spectrum-%i_7_8.txt",k);
  ...
  TLine *zero = new TLine(-0.3,1.0,1.05,1.0);
  zero->SetLineWidth(3);
  zero->SetLineStyle(9);
  ...
  f_out->Close();
  cfin->SaveAs("plot_err/stat_numu.pdf");
}

void ALL_systcut_Numu(){
  ...
  std::ifstream true_array("outputs_systcut_Numu/Unf_spectrum-95_7_8.txt");
  std::ifstream bins("../../matrix/likelihood/5YRS/output_Numu_osc/energybins_Numu_7.txt");
  std::ofstream rel_err("outputs_systcut_Numu/systcut_err_Numu_7_8.txt");
  TFile *f_out = new TFile("outputs_systcut_Numu/systcut_Numu_7_8.root", "RECREATE");
  ...
  for (int k=0; k<=10; k++) {
    index = 90+k;
    sprintf(input_file,"outputs_systcut_Numu/Unf_spectrum-%i_7_8.txt",index);
  ...
  TLegend *leg1 = new TLegend(0.15,0.07,0.27,0.75);
  ...
  for (int rr=1; rr<=10; rr++) {
    ...
    if (rr==5) {
      arr_graphs[rr]->SetMarkerColor(kBlue);
  ...
  for (int rr=1;rr<=10;rr++) {                                                                                                          
    ...
    if (rr==5) {
      arr_ratios[rr]->SetMarkerColor(kBlue);
  ...
  f_out->Close();
  cfin->SaveAs("plot_err/systcut_numu.pdf");
}
